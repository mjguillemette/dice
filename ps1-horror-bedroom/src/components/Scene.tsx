...
import { useInput, useMouseLook, requestPointerLock } from '../systems/inputSystem';
...
export function Scene({ onCameraNameChange, onCameraModeChange, onHellFactorChange, onAutoCorruptionChange, diceCount, coinCount, d3Count, d4Count, thumbTackCount, onDiceScoreChange, diceShaderEnabled }: SceneProps) {
  ...
  const handleClick = useCallback((event: MouseEvent) => {
    ...
    if (document.pointerLockElement) {
      ...
      const didIntersect = raycaster.ray.intersectPlane(floorPlane, intersectPoint);

      console.log('Floor intersect:', didIntersect, intersectPoint);

      if (didIntersect && diceManagerRef.current) {
        console.log('Throwing dice from camera at:', cameraPosition, 'to:', intersectPoint);
  ...
  return (
    <>
      ...
      <Physics gravity={[0, -9.81, 0]}>
        ...
        <RigidBody type="fixed" position={[0, -0.05, 0]} friction={0.8} restitution={0.2}>
        ...
        <RigidBody type="fixed" position={[-2, 0.4, -2]} friction={0.6} restitution={0.3}>
          <CuboidCollider args={[1.6, 0.5, 2.1]} />
        </RigidBody>

        {/* Bureau - Including drawers */}
        <RigidBody type="fixed" position={[3, 0.8, -4.1]} friction={0.5} restitution={0.2}>
          <CuboidCollider args={[0.9, 0.6, 0.55]} />
        </RigidBody>

        {/* TV Stand - Combined bounding box */}
        <RigidBody type="fixed" position={[0, 0.9, -4.4]} friction={0.5} restitution={0.2}>
          <CuboidCollider args={[1.25, 1.0, 0.4]} />
        </RigidBody>

        {/* Coffee Table - In second room */}
        <RigidBody type="fixed" position={[0, 0.5, 10]} friction={0.5} restitution={0.2}>
          <CuboidCollider args={[0.9, 0.25, 0.6]} />
        </RigidBody>

        {/* Dice Receptacle - THICK solid base to prevent tiny dice tunneling */}
        <RigidBody type="fixed" position={[1.5, 0.0, 2.5]} friction={0.8} restitution={0.15} ccd={true}>
